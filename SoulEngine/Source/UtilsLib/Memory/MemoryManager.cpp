...
namespace Soul
{
	...
	UInt32 MemoryManager::_uiHandleTableLength = 256;
	...
	void MemoryManager::StartUp(ByteCount uiByteSize)
	{
		...
		*/
		...
		_suipAddressableMemoryStart = (Byte*)(_sopHandleTableStart + _uiHandleTableLength);
		_sbIsSetup = true;

		memset(_sopHandleTableStart, 0, sizeof(Handle) * _uiHandleTableLength);
	}
		...
		Handle** opPreviousHandleOut)
	{
		...
		while (opNextHandle)
		{
			...
			*/
			...
			if ((Byte*)opNextHandle->uipLocation - uipEndOfBlock >= uiRequestedSize)
			{
		...
		*/
		Byte* uipEndOfBlock =
			(Byte*)opCurrentHandle->uipLocation + opCurrentHandle->uiByteSize;
		if (_suipMemoryEnd - uipEndOfBlock >= uiRequestedSize)
		{
			(*opPreviousHandleOut) = opCurrentHandle;
			return uipEndOfBlock;
		}
